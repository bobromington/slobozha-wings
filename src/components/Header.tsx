    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="container flex items-center justify-between h-16 md:h-20">
        <Link to="/" className="flex items-center gap-3">
          <img src={logo} alt="Sloboda Unit Logo" className="h-[38px] md:h-[53px] w-auto" />
          <img src={slobodaSamosud} alt="Sloboda Samosud Team wordmark" className="h-[30px] md:h-[42px] w-auto -translate-y-[1px] md:-translate-y-[1px]" />
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            aria-label={open ? (lang === 'ua' ? 'Закрити меню' : 'Close menu') : (lang === 'ua' ? 'Відкрити меню' : 'Open menu')}
            aria-expanded={open}
          >
